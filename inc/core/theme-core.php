...
function cactusthemes_comment( $comment, $args, $depth ) {
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		<article id="comment-<?php comment_ID(); ?>" class="comment">
			...
			<?php 
			$author = get_comment( $cmt );
            $a_id=$author->user_id;
			$user_info = get_userdata( $a_id);
			
			$lk= $user_info->user_login;
			$media=site_url().'/members/'.$lk.'/media/';
			echo '<a href="'.$media.'">';
			...
			<div class="comment-meta comment-author">
				<?php
					...
					printf( '<cite class="fn">%1$s</cite> ', get_comment_author());
				?>
