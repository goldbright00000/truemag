						  ...
						  <div class="item-thumbnail">
							...
							if(has_post_thumbnail()){
                                ...
                                if($responsive_mode == 'on'){
                                    $html .= cactus_thumbnail('thumb_139x89');
                                } else {
		...
		function get_item_video($thumb, $show_title, $show_rate, $show_dur, $themes_pur, $quick_view='def'){	
		  ...
		  $html .= '
				<div class="qv_tooltip"  title="
					<h4 class=\'gv-title\'>'.esc_attr($title).'</h4>
					<div class=\'gv-ex\' >'.esc_attr(get_the_excerpt()).'</div>
					<div class= \'gv-button\'>';
						if($format == 'video'){
							$html .= '<div class=\'quick-view\'><a href='.get_permalink().' title=\''.esc_attr($title).'\'>'.__('Watch Now','cactusthemes').'</a></div>';
						}else{
							$html .= '<div class=\'quick-view\'><a href='.get_permalink().' title=\''.esc_attr($title).'\'>'.__('Read more','cactusthemes').'</a></div>';
						}
						$html .='
			...
			$html .= '	
				<div class="video-item">
					<div class="item-thumbnail">
						 <a href="'.get_permalink().'" title="'.esc_attr($title).'">';
							if(has_post_thumbnail()){
                                $responsive_mode = ot_get_option('responsive_image', 'on');
                                if($responsive_mode == 'on'){
                                    $html .= cactus_thumbnail($thumb, $post_id);
                                } else {
						...
						$html .= '</a>';
						if($show_rate != '0'){
                            $html .= tm_post_rating($post_id);
						}
						if($show_dur!='0'){
                            $time_video = get_post_meta($post_id,'time_video',true);
                            if($time_video != '00:00' && $time_video != '00' && $time_video != '' ){
                            $html .= '
                                <span class="rating-bar bgcolor2 time_dur">'.get_post_meta($post_id,'time_video',true).'</span>';
                            }
						}
						if($show_title != '0'){
						$html .= '
						<div class="item-head">
							<h3><a href="'.get_permalink($post_id).'">'.strip_tags(get_the_title($post_id)).'</a></h3>
						</div>';}
				$html .= '</div>
				</div>';
				if($quick_if=='1'){
				...
				$html .= '
				  <div class="item-thumbnail">
					  ...
					  if($show_dur!='0'){
						...
						if($time_video != '00:00' && $time_video != '00' && $time_video != '' ){
								$html .= '
								<span class="rating-bar bgcolor2 time_dur">'.$time_video.'</span>';
							}
				...
				$html .= '
				  <div class="item-thumbnail">
					  ...
					  if($show_dur!='0'){
						...
						if($time_video != '00:00' && $time_video != '00' && $time_video != '' ){
							$html .= '<span class="rating-bar bgcolor2 time_dur">' . $time_video . '</span>';
						}
					  }
				  $html .= '
				  </div>';
				  ...
				  if($show_title!='0'){
				  $html .= '
					  <h3><a href="'.get_permalink($time_video).'">'.strip_tags(get_the_title($time_video)).'</a></h3>';
				  }
				  $html .= '</div>
				  </div>';
		return $html;
							...
							<div class="item-thumbnail">
								  <a href="'.get_permalink($post->post_id).'" title="'.the_title_attribute('echo=0').'">';
                                      if(has_post_thumbnail($post->post_id)){
                                            $responsive_mode = ot_get_option('responsive_image', 'on');
                                            if($responsive_mode == 'on'){
                                                $widget_data .= cactus_thumbnail('thumb_139x89', $post->post_id);
                                            } else {
